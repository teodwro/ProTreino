...
const AddTreino = () => {
  ...
  const salvarTreino = async () => {
    if (!diaSemana || !pchId || selectedExercicios.length === 0) {
      Alert.alert('Erro', 'Preencha todos os campos e selecione pelo menos 1 exercício.');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        Alert.alert('Sucesso', treinoId ? 'Treino atualizado com sucesso!' : 'Treino salvo com sucesso!');
        router.back();
      } else {
        const json = await res.json();
        console.error(json);
        Alert.alert('Erro', json.message || 'Não foi possível salvar o treino.');
      }
    } catch (err) {
      console.error(err);
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.pickerWrapper}>
        ...
        >
          ...
          {pchList.map(pch => (
            <Picker.Item key={pch.id} label={pch.nome} value={pch.id.toString()} />
          ))}
        </Picker>
      </View>
            ...
            selectedExercicios.map((ex) => (
              <View key={ex.id} style={styles.exercicioBox}>
                ...
                <TextInput
                  ...
                  keyboardType="numeric"
                  value={ex.repeticoes}
                  onChangeText={(val) => atualizarCampo(ex.id, 'repeticoes', val)}
                />
                <TextInput
                  style={styles.input}
